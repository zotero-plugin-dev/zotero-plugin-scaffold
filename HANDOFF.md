...
8. **vitest 升级检查**：页面全局注入走官方 `setupCommonEnv`（`vitest/internal/browser`，start 时调用，强制 `config.globals = true`）——与 `globals: true` 同一代码路径，无手写名单；`WorkerStateLike` 有编译期契约（`keyof` 必须 ⊆ 官方 `WorkerGlobalState`，字段改名/删除会 tsc 报错）。升级 vitest 后：跑 scaffold 单测（bundler.test 覆盖打包）+ 验证项目真机即可，无需手改列表。
9. 已知坑（勿重踩）：**模板字符串拼接 PowerShell 脚本时，注释会诱发 eslint --fix 重排成 ...（一元加 → NaN 插值，脚本静默失败 → Zotero 残留）——用数组 join 拼接**；python 字符串替换在 eslint 格式化后静默失败（改文件用 write 或行级匹配）；`\\n` 经工具层转义（用 `chr(92)+"n"`）；eslint --fix 会重排 if/import（替换前先看实际格式）；Windows 下编辑器会写 CRLF（`core.autocrlf=input` 下 `git diff` 报假差异，提交前先转 LF）；**改页面/协议相关代码后 dist 需 `pnpm build:tsdown` 重建**（验证项目 symlink 直接吃 dist，且 `dist/core/tester/page/*.ts` 是源码拷贝）；execSync 传含嵌套引号的 powershell 命令会被 cmd 吞掉（用 `-EncodedCommand`）；git 会把含 NUL 字节的文件当二进制（虚拟模块 id 用 `\0` 转义常量而非字面 NUL）
