...
8. **vitest 升级检查**：页面全局注入名单不再手写——bundler 构建期从 vitest 的 `globalApis` 常量提取（`chunks/constants.*.js` 正则），提取失败回退内置名单并 warn；`WorkerStateLike` 有编译期契约（`keyof` 必须 ⊆ 官方 `WorkerGlobalState`，字段改名/删除会 tsc 报错）。升级 vitest 后：跑 scaffold 单测（bundler.test 覆盖打包）+ 验证项目真机即可，无需手改列表。
