...
export default class Test extends Base {
  ...
  async run(): Promise<void> {
    ...
    // legacy test.waitForPlugin expression becomes a setupFile the pool
    // bundles into the test page: it polls the plugin's ready flag before
    // any test runs (the wait shows up in vitest's "setup" duration).
    const waitForPlugin = this.ctx.test.waitForPlugin?.trim();
    if (waitForPlugin && waitForPlugin !== "() => true") {
      const setupPath = join(process.cwd(), ".scaffold", "setup-wait-plugin.js");
      const nl = String.fromCharCode(10);
      await outputFile(setupPath, [
        "// Generated by zotero-plugin test — waits for the plugin to be ready (test.waitForPlugin).",
        `const ready = ${waitForPlugin};`,
        "const deadline = Date.now() + 30000;",
        "while (!ready()) {",
        "  if (Date.now() > deadline) throw new Error(\"Timed out waiting for the plugin (test.waitForPlugin)\");",
        "  await new Promise((r) => setTimeout(r, 100));",
        "}",
        "",
      ].join(nl));
      logger.debug(`Generated plugin-ready setup file at ${setupPath}`);
    }
